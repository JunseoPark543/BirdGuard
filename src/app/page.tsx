...
export default function Home() {
  const [activeTab, setActiveTab] = useState<"home" | "analysis" | "buddyzone">("home");
  ...
  return <main className="site-shell">
    <header className="topbar"><button className="brand" type="button" onClick={() => navigateTo("home")}><span className="brand-mark"><Bird size={20} /></span><span><b>BirdGuard</b></span></button><button className="icon-button" type="button" aria-label="메뉴"><Menu /></button></header>
    <div className={activeTab === "home" ? "content" : "content content-compact"}>
      {activeTab === "home" && <section className="hero"><div className="hero-copy"><span className="eyebrow">BIRD-SAFE ARCHITECTURE</span><h1>조류 충돌 예방,<br /><em>버드가드가 지켜드려요!</em></h1><p>건물 사진 한 장으로 위험 요인을 분석하고, 공간에 꼭 맞는 충돌 방지 디자인을 제안합니다.</p><div className="hero-chips"><span>AI 위험도 진단</span><span>5×10cm 기준 설계</span><span>맞춤 디자인</span></div><button type="button" className="primary-button home-analysis-button" onClick={() => navigateTo("analysis")}><ScanLine /> 건물 사진 분석하기</button></div><div className="hero-visual"><span className="mascot-halo" aria-hidden="true" /><img src="/mascot/birdguard-front.png" alt="5×10 안전모를 쓴 버드가드 마스코트" /></div></section>}
      {activeTab === "analysis" && <>
      {step === "upload" && <div className="page-title"><div><span>인공지능 건물 분석</span><h1>건물 사진 분석하기</h1><p className="analysis-intro">건물 사진을 올리면 조류 충돌 위험을 분석하고 맞춤 디자인을 추천해 드려요.</p></div></div>}
      ...
      {activeTab === "buddyzone" && <KakaoBuddyzoneMap onBack={() => navigateTo("home")} />}
      {activeTab === "home" && <section className="why-section"><p className="section-kicker">WHY BIRDGUARD</p><h2>왜 버드가드인가요?</h2><div className="benefit-grid"><article><ScanLine /><b>5×10cm 법칙 준수</b><p>과학적으로 검증된 안전 간격</p></article><article><Bird /><b>건물 외벽과 완벽한 조화</b><p>재질/질감 맞춤 디자인 추천</p></article><article><MapPinned /><b>실제 설치 &amp; 인증까지</b><p>버디존 등록하고 인증 받으세요!</p></article></div></section>}
    </div>
    <nav className="bottom-nav" aria-label="주 메뉴"><button type="button" className={activeTab === "home" ? "active" : ""} aria-current={activeTab === "home" ? "page" : undefined} onClick={() => navigateTo("home")}><HomeIcon /><span>홈</span></button><button type="button" className={activeTab === "analysis" ? "active" : ""} aria-current={activeTab === "analysis" ? "page" : undefined} onClick={() => navigateTo("analysis")}><ScanLine /><span>분석하기</span></button><button type="button" className={activeTab === "buddyzone" ? "active" : ""} aria-current={activeTab === "buddyzone" ? "page" : undefined} onClick={() => navigateTo("buddyzone")}><MapPinned /><span>버디존</span></button><button type="button"><UserRound /><span>마이페이지</span></button></nav>
  </main>;
}
