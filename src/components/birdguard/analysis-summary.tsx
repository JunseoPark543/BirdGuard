...
export function AnalysisSummary({ analysis }: { analysis: BuildingAnalysis }) {
  ...
  return <div className="analysis-summary">
    ...
    <section><h2><span>3</span> 위험도 진단</h2><div className={`risk-card risk-${analysis.birdCollisionRisk}`}><ShieldAlert /><div><small>MASTER 분류 기준 종합</small><h3>충돌 위험도: {risks[analysis.birdCollisionRisk]}</h3><p>{analysis.riskFactors.join(" · ")}</p></div></div><div className="caution-card"><AlertTriangle /><div><b>설계 적용 시 확인하세요</b><p>{analysis.caution}</p><p><strong>패턴 간격은 가로 10cm, 세로 5cm 이하</strong>로 유지해야 합니다.</p></div></div></section>
