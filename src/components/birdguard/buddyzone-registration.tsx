...
export function BuddyzoneRegistration({ onBack }: { onBack: () => void }) {
  ...
  const ready = Boolean(before && after && store.trim() && address.trim() && category);

  if (submitted) return <KakaoBuddyzoneMap storeName={store} address={address} category={category} onBack={() => setSubmitted(false)} />;
  ...
  return <section className="buddyzone-page">
    ...
    <section className="registration-section"><div className="registration-section-title"><span>2</span><div><h2>매장 정보 입력</h2><p>버디존 지도에 표시될 정보를 입력해 주세요.</p></div></div><div className="registration-form"><label><span>매장명</span><input value={store} onChange={e => setStore(e.target.value)} placeholder="매장명을 입력해 주세요" /></label><label><span>주소</span><div><input value={address} onChange={e => setAddress(e.target.value)} placeholder="주소를 검색해 주세요" /><MapPin /></div></label><label><span>카테고리</span><div><select value={category} onChange={e => setCategory(e.target.value)}><option value="" disabled>카테고리 선택</option><option>카페</option><option>상점</option><option>학교</option><option>공공기관</option><option>기타</option></select><ChevronDown /></div></label></div></section>

    <button type="button" className="primary-button registration-submit" disabled={!ready} onClick={() => setSubmitted(true)}>다음 단계</button>
