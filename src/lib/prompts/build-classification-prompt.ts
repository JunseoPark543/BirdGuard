...
11. 원근 보정을 위해 서로 다른 방향의 입면을 하나로 묶지 않는다. 한 평면의 유리창 또는 패널 단위로 구분하고, 사각형 창문은 왼쪽 위 → 오른쪽 위 → 오른쪽 아래 → 왼쪽 아래 순서의 네 모서리를 우선 반환한다. 가려진 모서리를 임의로 만들어내지 않는다.
12. 서로 떨어진 유리 면은 별도 영역으로 나누고, 유리 영역을 신뢰성 있게 찾지 못하면 glassRegions를 빈 배열로 반환한다. 사진 전체나 건물 전체를 유리 영역으로 지정하지 않는다.
